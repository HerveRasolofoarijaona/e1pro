...
function newMessage() {
    ...
    $('<button type="button" data-toggle="modal" data-target="#myModal2" class="button btn1">Afficher la proposition</button>').appendTo($('.messages ul'));
    //$('.message-input input').val(null);
    //$('.contact.active .preview').html('<span>You: </span>' + message);
    //$(".messages").animate({ scrollTop: $(document).height() }, "fast");
};
$(".saveNegociation").submit(function (event) {
...
$(".refuse").submit(function (event) {
    // Prevent the form from submitting via the browser.
    event.preventDefault();
    refuse();
});
$(".approuved").submit(function (event) {
    // Prevent the form from submitting via the browser.
    event.preventDefault();
    approuved();
});
$(".not_approuved").submit(function (event) {
    // Prevent the form from submitting via the browser.
    event.preventDefault();
    notApprouved();
});
$(".dmd_approuved").submit(function (event) {
    // Prevent the form from submitting via the browser.
    event.preventDefault();
    dmd_aprouved();
});
...
//$(document).ready(function () {
    ...
    function ajaxPost() {
        // PREPARE FORM DATA
        var formData = {
            motif_1: $("input[name=motif1]").is(":checked"),
            motif_2: $("input[name=motif2]").is(":checked"),
            motif_3: $("input[name=motif3]").is(":checked"),
            date_db: $("input[name=dt_db]").val(),
            date_fn: $("input[name=dt_fn]").val(),
            prix: $("input[name=prix]").val(),
            sender: $("input[name=sender]").val()
        };
        //console.log($("input[name=devis_id]").val());
        //à remettre pour envoyer les données
        $.ajax({
            type: "PUT",
            contentType: "application/json",
            url: "/api/conversation/update/" + $("input[name=devis_id]").val() ,
            data: JSON.stringify(formData),
            dataType: 'json',
            success: function () {
                swal("Modification prise en compte", "Vous pouvez fermer le formulaire", "success");
                //notifNego();
                location.reload();
                //newMessage();
            },
    ...
    function accept() {
        // PREPARE FORM DATA
        var formData = {
            accept: $("input[name=accepted]").val(),
            negociate: $("input[name=negocier]").val(),
            attente: $("input[name=attente]").val(),
        };
        $.ajax({
            type: "PUT",
            contentType: "application/json",
            url: "/api/devis/" + $("input[name=devis_id]").val(),
            data: JSON.stringify(formData),
            dataType: 'json',
            success: function () {
                swal("Négociation acceptée","", "success");
                location.reload();
            },
            error: function (e) {
                alert("Error!")
    ...
    function refuse(){
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function () {
                swal("Négociation refusée", "", "error");
                //location.reload();
            },
            error: function (e) {
                alert("Error!")
...
//gérer boutons approuver et refuser de devis-details.jade
function approuved(){
    var formData = {
        approuved: "true",
    };
    $.ajax({
        type: "PUT",
        contentType: "application/json",
        url: "/api/devis/approuved/" + $("input[name=devis_id]").val(),
        data: JSON.stringify(formData),
        dataType: 'json',
        success: function () {
            swal("Négociation approuvée", "", "success");
            location.reload();
        },
        error: function (e) {
            alert("Error!")
...
function notApprouved() {
    var formData = {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function () {
                swal("Négociation non-approuvée", "", "error");
                location.reload();
            },
            error: function (e) {
                alert("Error!")
...
function dmd_aprouved() {
    var formData = {
        id_demand: $("input[name=id_demand]").val(),
    };
    $.ajax({
        type: "PUT",
        contentType: "application/json",
        url: "/api/demands/approuve/" + $("input[name=id_demand]").val(),
        data: JSON.stringify(formData),
        dataType: 'json',
        success: function () {
            swal("Demande approuvée", "", "success");
            location.reload();
        },
        error: function (e) {
            swal("OUPS Problème Server", "", "error");
        }
    });
}